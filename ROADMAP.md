...
- **跨端一套码**：Flutter 覆盖 Android + Windows，iOS/Mac 留 roadmap。
...
| 4 | 首发平台 | **Android + Windows**；iOS/Mac → roadmap |
...
### M3 — 安卓分享菜单（第二条腿）
**交付物：** Android 端出现在系统「分享到」菜单，接收 App 分享进来的链接/文本。
**完成标准：** 在手机微信/X/小红书点「分享 → Roosty」，内容进入同一条管线，归巢成 .md（vault 路径用 SAF 授权目录）。

### M4 — 开源发布（抢先达成）★项目目标
**交付物：** 公开 GitHub 仓库 + README + 配置示例 + 一键运行说明 + 演示 GIF。
**完成标准：** 陌生人按 README 能在 30 分钟内跑起来；仓库挂上 roosty.io（或 .dev）。
...
- [ ] ✅ T4.5 打包 Windows 安装包 + Android APK 挂 Release
...
| iOS 上架审核拖慢节奏 | 数周延误 | iOS 完全不进首发，安卓发 APK 免审核 |
| 移动端 vault 沙盒访问受限 | 安卓写入失败 | 安卓用 SAF 授权目录；iOS 后续单独解 |
