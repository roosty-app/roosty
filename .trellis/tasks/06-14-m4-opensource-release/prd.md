...
**唯一依据：项目根目录 `ROADMAP.md`（第 1 章目标 + 第 6 章风险）。**
**前置：M1（桌面闭环）必须完成；M2/M3 完成更好，但只要 M1 能 demo 即可发首版。**

---

## 目标

把 Roosty 做成陌生人能在 30 分钟内 `git clone` 跑起来的开源项目，抢在对方（Sherlockdogs）发布前公开传播。

## 完成标准（验收）

- 陌生人按 README 能在 30 分钟内跑起桌面端核心闭环。
- 仓库有清晰定位、截图、配置说明、LICENSE。
- Windows 安装包 + Android APK 挂在 Release。

## 节点 checklist

- [ ] T4.1 README：定位（一句话魔法）、核心截图/GIF、30 分钟上手步骤、vault 配置说明、LLM 配置说明、roadmap（iOS/浏览器扩展/平台专用抓取）
- [ ] T4.2 配置示例文件（`config.example` 之类）+ `.gitignore`（**确保任何含 key 的本地配置不进仓库**）
- [ ] T4.3 演示 GIF / 短视频：录核心魔法「复制链接 → 几秒后 Obsidian 出现带摘要的笔记」
- [ ] T4.4 LICENSE（MIT）+ 注册 `roosty.io`（或 `.dev`）+ 建 GitHub 组织（`roosty-io` / `roostyhq`，因 `roosty` 用户名已被占）
- [ ] T4.5 打包：Windows 安装包（`flutter build windows`）+ Android APK（`flutter build apk`），挂 GitHub Release
...
更新 checklist → `task.py finish`。项目核心目标（抢先开源）达成。
