...
- 注册自定义域名暂缓（2026-06-23 决定）：`roosty.cn` 已在阿里云查证 ¥38 首年 / ¥42 续费，可注册；但项目有无社区反响未知，**先 push 仓库看反响再决定是否花钱**。门槛低（38 元，3 天内可下单），不阻塞首发。`roosty.com` / `.net` 已被注册，`.io` 太贵不考虑。这些需要用户的账号和手动操作；M4 阶段 agent 只能把文件（README/LICENSE/配置示例/打包脚本）准备好。
